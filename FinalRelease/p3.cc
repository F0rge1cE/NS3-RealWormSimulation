...
int main(int argc, char* argv[])
{
  ...
  string hubInnerRate ("1Mbps");
  string innerChildRate ("1Mbps");
  string backBoneRate ("50Mbps");
  string backGroundTraffic ("1Kbps");
  ...
  // more parameter
  cmd.AddValue ("hubInnerRate",      "Hub to inner nodes connection speed",      hubInnerRate);
  cmd.AddValue ("innerChildRate",      "Inner to child nodes connection speed",  innerChildRate);
  cmd.AddValue ("backBoneRate",      "Hub to hub speed",                    backBoneRate);
  cmd.AddValue ("backGroundTraffic",      "Background Traffic rate",     backGroundTraffic);
  ...
  hubInner.SetDeviceAttribute("DataRate", StringValue(hubInnerRate));
  hubInner.SetChannelAttribute("Delay", StringValue("5ms"));

  PointToPointHelper innerChild;
  innerChild.SetDeviceAttribute("DataRate", StringValue(innerChildRate));
  ...
  hub2hub.SetDeviceAttribute("DataRate", StringValue(backBoneRate));
  ...
  for (uint32_t i = 0; i<bombs.size(); i++){
    for(uint32_t j = 0; j < nChild * nInner; j++){
      ...
      client.SetAttribute ("DataRate",StringValue (backGroundTraffic));
